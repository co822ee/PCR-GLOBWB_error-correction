...
if(!dir.exists(paste0('../graph/', calibrMod, '/'))){
    paste0('../graph/', calibrMod, '/') %>% dir.create()
}
...
for(station_i in 1:length(station)){
    ...
    p1
    ggsave(paste0(paste0('../graph/', calibrMod,'/cor_state_res_', plotTitle, '.tiff')), dpi=300, 
           width=10, height=6.5)
...
for(station_i in 1:length(station)){
    ...
    for(j in 1:(length(feature)%%r-1)){
        ...
        ggsave(paste0(paste0('../graph/', calibrMod,'/timeSeries_pred',j,'_', plotTitle,'.tiff')), dpi=300,
               width=7, height=7.5)
    }
}
...
for(id in 1:length(station)){
    ...
    plotList[[id]] <- ggplot(data=allP, 
        ...
        facet_grid(datatype~.)+
        labs(x='', y='residuals (cms)', title = plotTitle)+
        scale_x_date(date_labels = '%b', date_breaks = '1 month')
...
for(id in 1:length(station)){
    ...
    plotList[[id+5]] <- ggplot(data=allP, 
        ...
        geom_ribbon(data=allP %>%
                    ...
                    fill='light blue', alpha=0.1)+
        facet_grid(datatype~.)+
        labs(x='', y='observations (cms)', title = plotTitle)+
        scale_x_date(date_labels = '%b', date_breaks = '1 month')
    if(id!=1) plotList[[id+5]] <- plotList[[id+5]]+labs(y='')
...
for(id in 1:length(station)){
    ...
    plotList[[id+10]] <- ggplot(data=allP, 
        ...
        scale_x_date(date_labels = '%b', date_breaks = '1 month')
    if(id!=1) plotList[[id+10]] <- plotList[[id+10]]+labs(y='')
    
}
plotList$nrow <- 3

tiff(paste0('../graph/', calibrMod,'/timeSeries.tiff'), 
     height=8, width=18, units='in', res=300)
