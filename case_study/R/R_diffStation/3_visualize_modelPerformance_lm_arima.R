...
files <- lapply(dir, list.files, pattern='lmarima_eval')
...
ggplot(data = eval_allG %>% 
  ...
  labs(     #title = 'Model performance at different stations', 
    ...
    fill=paste0('Models'))
ggsave('../graph/RFresult_all/gof_abs_new_test_lm.tiff', dpi = 300,
       width = 7.5, height = 6)
...
ggplot(data = eval_allG %>% 
  ...
  labs(     #title = 'Model performance at different stations', 
    ...
    color=paste0('PCR without RF-correction'), 
    fill=paste0('Models'))
ggsave('../graph/RFresult_all/gof_abs_new_train_lm.tiff', dpi = 300,
       width = 7.5, height = 6)
